...
int COGS_MAIN()
{
	return cogs::main([](const rcref<gui::windowing::subsystem>& guiSubsystem)
	{
...

		{
			...
			//grn.set_alpha(0x20); 
			//rcref<background> backgroundPane = rcnew(background, grn);
			//backgroundPane->set_compositing_behavior(compositing_behavior::buffer_self_and_children);
			...
			drawPane->set_compositing_behavior(compositing_behavior::buffer_self_and_children);
		...
		//{
		//	rcref<background> backgroundPane = rcnew(background, color::purple);
		//	backgroundPane->set_compositing_behavior(compositing_behavior::buffer_self_and_children);
		//	rcref<bitmap_pane> img4 = rcnew(bitmap_pane, string::literal(L"guitar"));// , true);
