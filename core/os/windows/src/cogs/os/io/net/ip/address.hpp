...
public:
	...
	protected:
		...
		explicit lookup_result(const composite_string& s, const rcref<network>& n = network::get_default())
	...
	protected:
		...
		explicit reverse_lookup_result(const address& addr, const rcref<network>& n = network::get_default())
			: m_network(n)
		{
...
//	}
	...
	{
		return rcnew(lookup_result)(addr);
	}
