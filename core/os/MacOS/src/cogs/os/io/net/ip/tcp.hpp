			...
			if (m_completionSerializer.append(taskType))
			{
				for (;;)
				{
					...
					else // if (taskType == abort_task)
					{
						...
						{
							m_aborted = true;
							m_socket->m_kqueuePool->abort_read_waiter(m_waiterRemoveToken);
							complete();
						}
					}
					bool wasLast;
			...
			if (m_completionSerializer.append(taskType))
			{
				for (;;)
				{
					...
					else // if (taskType == abort_task)
					{
						...
						{
							m_aborted = true;
							m_socket->m_kqueuePool->abort_write_waiter(m_waiterRemoveToken);
							complete();
						}
					}
					bool wasLast;
...
public:
	...
	static rcref<connecter> connect(const address& addr, unsigned short port, const rcref<os::io::kqueue_pool>& kq = os::io::kqueue_pool::get())
	{
		...
		return connect(addresses, port, kq);
	}
