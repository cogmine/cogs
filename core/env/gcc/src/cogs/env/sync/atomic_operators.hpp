...
exchange(volatile T& t, const T& src, T& rtn)
{
	...
	rtn = (T)__atomic_exchange_n((uint_t*)&t, (uint_t)src, __ATOMIC_SEQ_CST);
}
...
exchange(volatile T& t, const T& src)
{
	COGS_ASSERT((size_t)&t % atomic::get_alignment_v<T> == 0);
	typedef bytes_to_uint_t<sizeof(T)> uint_t;
	return (T)__atomic_exchange_n((uint_t*)(unsigned char*)&t, (uint_t)src, __ATOMIC_SEQ_CST);
}
...
compare_exchange(volatile T& t, const T& src, const T& cmp, T& rtn)
{
	...
	typedef bytes_to_uint_t<sizeof(T)> uint_t;
	rtn = cmp;
	return __atomic_compare_exchange_n((uint_t*)&t, (uint_t*)&rtn, (uint_t)src, false, __ATOMIC_SEQ_CST, __ATOMIC_SEQ_CST);
}
