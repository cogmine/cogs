...
/// @brief A sorted O(log n) collection.  Duplicate values are allowed.
...
private:
	typedef multiset<type, comparator_t, memory_manager_type> this_t;
	...
	typedef container_skiplist<true, type, payload, comparator_t, memory_manager_type> container_skiplist_t;
...
public:
	...
	/// @brief A multiset element iterator
	class iterator
	{
	private:
	...
	/// @brief A volatile multiset element iterator
	class volatile_iterator
	{
	private:
		friend class multiset;
	...
	/// @brief A multiset element remove_token
	class remove_token
	{
	private:
		friend class multiset;
	...
	/// @brief A volatile multiset element remove_token
	class volatile_remove_token
	{
	private:
		friend class multiset;
	...
	multiset(args_t&&... args)
		: m_contents(std::forward<args_t>(args)...)
	{ }

	multiset(const volatile this_t&) = delete;
	...
	std::enable_if_t<
		...
		iterator>
	insert_via(F&& f) { return insert_via([&](iterator& i) { f(*const_cast<type*>(&i.get_key())); }); }
	...
	std::enable_if_t<
		...
		&& std::is_invocable_v<F, type&>,
		volatile_insert_result>
	insert_via(F&& f) volatile { return insert_via([&](iterator& i) { f(*const_cast<type*>(&i.get_key())); }); }
	...
	volatile_insert_result insert(const type& k) volatile
	{
		return insert_via([&](iterator& i)
		{
			nested_rcnew(&i.m_contents->get_key(), *i.get_desc())(k);
	...
	volatile_insert_result insert(type&& k) volatile
	{
		return insert_via([&](iterator& i)
		{
			nested_rcnew(&i.m_contents->get_key(), *i.get_desc())(std::move(k));
	...
	iterator operator+=(const type& k)
	{
		return insert_via([&](iterator& i)
		{
			nested_rcnew(&i.m_contents->get_key(), *i.get_desc())(k);
	...
	volatile_insert_result operator+=(const type& k) volatile
	{
		return insert_via([&](iterator& i)
		{
			nested_rcnew(&i.m_contents->get_key(), *i.get_desc())(k);
	...
	volatile_insert_result operator+=(type&& k) volatile
	{
		return insert_via([&](iterator& i)
		{
			nested_rcnew(&i.m_contents->get_key(), *i.get_desc())(std::move(k));
	...
	template <typename... args_t>
	iterator insert_emplace(args_t&&... args)
	{
		return insert_via([&](iterator& i)
		{
	...
	template <typename... args_t>
	volatile_insert_result insert_emplace(args_t&&... args) volatile
	{
		return insert_via([&](iterator& i)
		{
			nested_rcnew(&i.m_contents->get_key(), *i.get_desc())(std::forward<args_t>(args)...);
	...
	/// @{
	/// @brief Find any element with a matching value
	/// @param criteria The value to search for
	/// @return An iterator to the found element, or an empty iterator if not found.
	iterator find_any_equal(const type& criteria) const { return iterator(m_contents.find_any_equal(criteria)); }
	/// @brief Thread-safe implementation of find_any_equal()
	volatile_iterator find_any_equal(const type& criteria) const volatile { return volatile_iterator(m_contents.find_any_equal(criteria)); }
	/// @}

	/// @{
	/// @brief Find the first element with a matching value
	/// @param criteria The value to search for
	/// @return An iterator to the found element, or an empty iterator if not found.
	iterator find_first_equal(const type& criteria) const { return iterator(m_contents.find_first_equal(criteria)); }
	/// @brief Thread-safe implementation of find_first_equal()
	volatile_iterator find_first_equal(const type& criteria) const volatile { return volatile_iterator(m_contents.find_first_equal(criteria)); }
	/// @}

	/// @{
	/// @brief Find the last element with a matching value
	/// @param criteria The value to search for
	/// @return An iterator to the found element, or an empty iterator if not found.
	iterator find_last_equal(const type& criteria) const { return iterator(m_contents.find_last_equal(criteria)); }
	/// @brief Thread-safe implementation of find_last_equal()
	volatile_iterator find_last_equal(const type& criteria) const volatile { return volatile_iterator(m_contents.find_last_equal(criteria)); }
	/// @}
	...
	/// @param criteria The value to search for
	/// @return An iterator to the found element, or an empty iterator if not found.
	iterator find_any_equal_or_nearest_less_than(const type& criteria) const { return iterator(m_contents.find_any_equal_or_nearest_less_than(criteria)); }
	/// @brief Thread-safe implementation of find_any_equal_or_nearest_less_than()
	volatile_iterator find_any_equal_or_nearest_less_than(const type& criteria) const volatile { return volatile_iterator(m_contents.find_any_equal_or_nearest_less_than(criteria)); }
	/// @}
	...
	/// @param criteria The value to search for
	/// @return An iterator to the found element, or an empty iterator if not found.
	iterator find_any_equal_or_nearest_greater_than(const type& criteria) const { return iterator(m_contents.find_any_equal_or_nearest_greater_than(criteria)); }
	/// @brief Thread-safe implementation of find_any_equal_or_nearest_greater_than()
	volatile_iterator find_any_equal_or_nearest_greater_than(const type& criteria) const volatile { return volatile_iterator(m_contents.find_any_equal_or_nearest_greater_than(criteria)); }
	/// @}
	...
	/// @param criteria The value to search for
	/// @return An iterator to the found element, or an empty iterator if not found.
	iterator find_first_equal_or_nearest_less_than(const type& criteria) const { return iterator(m_contents.find_first_equal_or_nearest_less_than(criteria)); }
	/// @brief Thread-safe implementation of find_first_equal_or_nearest_less_than()
	volatile_iterator find_first_equal_or_nearest_less_than(const type& criteria) const volatile { return volatile_iterator(m_contents.find_first_equal_or_nearest_less_than(criteria)); }
	/// @}
	...
	/// @param criteria The value to search for
	/// @return An iterator to the found element, or an empty iterator if not found.
	iterator find_first_equal_or_nearest_greater_than(const type& criteria) const { return iterator(m_contents.find_first_equal_or_nearest_greater_than(criteria)); }
	/// @brief Thread-safe implementation of find_first_equal_or_nearest_greater_than()
	volatile_iterator find_first_equal_or_nearest_greater_than(const type& criteria) const volatile { return volatile_iterator(m_contents.find_first_equal_or_nearest_greater_than(criteria)); }
	/// @}
	...
	/// @param criteria The value to search for
	/// @return An iterator to the found element, or an empty iterator if not found.
	iterator find_last_equal_or_nearest_less_than(const type& criteria) const { return iterator(m_contents.find_last_equal_or_nearest_less_than(criteria)); }
	/// @brief Thread-safe implementation of find_last_equal_or_nearest_less_than()
	volatile_iterator find_last_equal_or_nearest_less_than(const type& criteria) const volatile { return volatile_iterator(m_contents.find_last_equal_or_nearest_less_than(criteria)); }
	/// @}
	...
	/// @param criteria The value to search for
	/// @return An iterator to the found element, or an empty iterator if not found.
	iterator find_last_equal_or_nearest_greater_than(const type& criteria) const { return iterator(m_contents.find_last_equal_or_nearest_greater_than(criteria)); }
	/// @brief Thread-safe implementation of find_last_equal_or_nearest_greater_than()
	volatile_iterator find_last_equal_or_nearest_greater_than(const type& criteria) const volatile { return volatile_iterator(m_contents.find_last_equal_or_nearest_greater_than(criteria)); }
	/// @}
...
class nonvolatile_multiset_node : public std::conditional_t<favor_lookup, avltree_node_t<nonvolatile_multiset_node<T, favor_lookup>>, rbtree_node_t<nonvolatile_multiset_node<T, favor_lookup>> >
{
public:
	typedef T type;
...
public:
	~nonvolatile_multiset_node() { m_value.destruct(); }
...
template <typename T, bool favor_lookup = false, class comparator_t = default_comparator, class allocator_t = batch_allocator<nonvolatile_multiset_node<T, favor_lookup>>>
class nonvolatile_multiset
{
public:
	typedef T type;
...
private:
	...
	typedef nonvolatile_multiset_node<type, favor_lookup> node;
...
public:
	...
	{
	private:
		friend class nonvolatile_multiset;
	...
	nonvolatile_multiset()
		: m_count(0)
	{ }

	template <typename arg1_t>
	nonvolatile_multiset(arg1_t&& arg1, std::enable_if_t<!std::is_convertible_v<std::remove_cv_t<arg1_t>, this_t>, internal_t> = {})
	{
		insert(std::forward<arg1_t>(arg1));
	}

	template <typename arg1_t, typename arg2_t, typename... args_t>
	nonvolatile_multiset(arg1_t&& arg1, arg2_t&& arg2, args_t&&... args)
	{
		insert(std::forward<arg1_t>(arg1));
	...
	nonvolatile_multiset(this_t&& src)
	...
	nonvolatile_multiset(const this_t& src)
	{
		for (const auto& entry : src)
			insert(entry);
	}
	...
	nonvolatile_multiset(const volatile this_t&) = delete;
	...
	std::enable_if_t<
		...
		&& std::is_invocable_v<F, type&>,
		iterator>
	insert_via(F&& f) { return insert_via([&](iterator& i) { f(i.m_node->get_key()); }); }

	iterator insert(const type& t) { return insert_via([&](iterator& i) { new (&i.m_node->get_key()) type(t); }); }

	iterator insert(type&& t) { return insert_via([&](iterator& i) { new (&i.m_node->get_key()) type(std::move(t)); }); }
	...
	void remove(const iterator& i)
	{
		...
		m_allocator.destruct_deallocate(i.m_node);
		m_count--;
	}

	iterator find_any_equal(const type& criteria) const { return iterator(m_tree.find_any_equal(criteria), m_tree); }
	iterator find_first_equal(const type& criteria) const { return iterator(m_tree.find_first_equal(criteria), m_tree); }
	iterator find_last_equal(const type& criteria) const { return iterator(m_tree.find_last_equal(criteria), m_tree); }
	...
	iterator find_any_equal_or_nearest_less_than(const type& criteria) const { return iterator(m_tree.find_any_equal_or_nearest_less_than(criteria), m_tree); }
	iterator find_any_equal_or_nearest_greater_than(const type& criteria) const { return iterator(m_tree.find_any_equal_or_nearest_greater_than(criteria), m_tree); }
	iterator find_first_equal_or_nearest_less_than(const type& criteria) const { return iterator(m_tree.find_first_equal_or_nearest_less_than(criteria), m_tree); }
	iterator find_first_equal_or_nearest_greater_than(const type& criteria) const { return iterator(m_tree.find_first_equal_or_nearest_greater_than(criteria), m_tree); }
