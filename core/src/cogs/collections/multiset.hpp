...
private:
	typedef multiset<type, comparator_t, allocator_type> this_t;
...
public:
	...
	multiset() { }
	multiset(volatile allocator_type& al) : m_contents(al) { }
	...
	volatile_iterator insert(const type& t) volatile
	{
		typename container_skiplist_t::preallocated i = m_contents.preallocate();
		i.get()->construct(t);
	...
	iterator insert(const type& t, bool& wasEmpty)
	{
		typename container_skiplist_t::preallocated i = m_contents.preallocate();
		i.get()->construct(t);
	...
	volatile_iterator insert(const type& t, bool& wasEmpty) volatile
	{
		typename container_skiplist_t::preallocated i = m_contents.preallocate();
		i.get()->construct(t);
	...
	preallocated preallocate(const type& v)
	{
		preallocated i = m_contents.preallocate();
	...
	const rcref<T2>& preallocate_with_aux(preallocated& i, unowned_t<rcptr<T2> >& storage = unowned_t<rcptr<T2> >().get_unowned())
	{
		m_contents.template preallocate_with_aux<T2>(i.m_contents, storage);
		i.get_payload().construct();
		return storage.dereference();
	}

	template <typename T2>
	const rcref<T2>& preallocate_with_aux(const type& v, preallocated& i, unowned_t<rcptr<T2> >& storage = unowned_t<rcptr<T2> >().get_unowned())
	{
		m_contents.template preallocate_with_aux<T2>(i.m_contents, storage);
		i.get_payload().construct(v);
		return storage.dereference();
	...
	iterator insert_preallocated(const preallocated& i) { return iterator(m_contents.insert_multi_preallocated(i.m_contents)); }

	volatile_iterator insert_preallocated(const preallocated& i) volatile { return volatile_iterator(m_contents.insert_multi_preallocated(i.m_contents)); }


	iterator insert_preallocated(const preallocated& i, bool& wasEmpty) { return iterator(m_contents.insert_multi_preallocated(i.m_contents, wasEmpty)); }

	volatile_iterator insert_preallocated(const preallocated& i, bool& wasEmpty) volatile { return volatile_iterator(m_contents.insert_multi_preallocated(i.m_contents, wasEmpty)); }
...
private:
	...
	nonvolatile_multiset(const this_t&) = delete;
	this_t& operator=(const this_t&) = delete;
...
public:
	...
	nonvolatile_multiset(volatile allocator_type& al)
		: m_count(0),
		m_allocator(al)
	{ }
