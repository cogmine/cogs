...
public:
	...
	preallocated_t preallocate(const type& v)
	{
		preallocated_t i = m_contents.preallocate();
	...
	const rcref<T2>& preallocate_with_aux(preallocated_t& i, unowned_t<rcptr<T2> >& storage = unowned_t<rcptr<T2> >().get_unowned())
	{
	...
	template <typename T2>
	const rcref<T2>& preallocate_with_aux(const type& v, preallocated_t& i, unowned_t<rcptr<T2> >& storage = unowned_t<rcptr<T2> >().get_unowned())
	{
		m_contents.template preallocate_with_aux<T2>(i.m_preallocated, storage);
		i.get_payload().construct(v);
		return storage.dereference();
