...
/// @ingroup Crypto
/// @brief Snefru Cryptographic Hash Algorithm
...
private:
	...
	void process_block()
	{
		...
		for (size_t i = 0; i < (16 - base_t::digest_digits); i++)
			w[i + base_t::digest_digits] = m_state[i];

		const uint32_t* sbox_end = sboxes + 4096;
		for (const uint32_t* sbox = sboxes; sbox < sbox_end; sbox += 512)
		{
			const uint32_t* sbox2 = sbox + 256;
			for (uint32_t rot = 0x18100810; rot; rot >>= 8)
			{
				uint32_t x = sbox[(unsigned char)(w[0])];
				w[15] ^= x;
				w[1] ^= x;
				x = sbox[(unsigned char)(w[1])];
				w[0] ^= x;
				w[2] ^= x;
				x = sbox2[(unsigned char)(w[2])];
				w[1] ^= x;
				w[3] ^= x;
				x = sbox2[(unsigned char)(w[3])];
				w[2] ^= x;
				w[4] ^= x;
				x = sbox[(unsigned char)(w[4])];
				w[3] ^= x;
				w[5] ^= x;
				x = sbox[(unsigned char)(w[5])];
				w[4] ^= x;
				w[6] ^= x;
				x = sbox2[(unsigned char)(w[6])];
				w[5] ^= x;
				w[7] ^= x;
				x = sbox2[(unsigned char)(w[7])];
				w[6] ^= x;
				w[8] ^= x;
				x = sbox[(unsigned char)(w[8])];
				w[7] ^= x;
				w[9] ^= x;
				x = sbox[(unsigned char)(w[9])];
				w[8] ^= x;
				w[10] ^= x;
				x = sbox2[(unsigned char)(w[10])];
				w[9] ^= x;
				w[11] ^= x;
				x = sbox2[(unsigned char)(w[11])];
				w[10] ^= x;
				w[12] ^= x;
				x = sbox[(unsigned char)(w[12])];
				w[11] ^= x;
				w[13] ^= x;
				x = sbox[(unsigned char)(w[13])];
				w[12] ^= x;
				w[14] ^= x;
				x = sbox2[(unsigned char)(w[14])];
				w[13] ^= x;
				w[15] ^= x;
				x = sbox2[(unsigned char)(w[15])];
				w[14] ^= x;
				w[0] ^= x;

				w[0] = bit_rotate_right(w[0], (unsigned char)rot);
				w[1] = bit_rotate_right(w[1], (unsigned char)rot);
				w[2] = bit_rotate_right(w[2], (unsigned char)rot);
				w[3] = bit_rotate_right(w[3], (unsigned char)rot);
				w[4] = bit_rotate_right(w[4], (unsigned char)rot);
				w[5] = bit_rotate_right(w[5], (unsigned char)rot);
				w[6] = bit_rotate_right(w[6], (unsigned char)rot);
				w[7] = bit_rotate_right(w[7], (unsigned char)rot);
				w[8] = bit_rotate_right(w[8], (unsigned char)rot);
				w[9] = bit_rotate_right(w[9], (unsigned char)rot);
				w[10] = bit_rotate_right(w[10], (unsigned char)rot);
				w[11] = bit_rotate_right(w[11], (unsigned char)rot);
				w[12] = bit_rotate_right(w[12], (unsigned char)rot);
				w[13] = bit_rotate_right(w[13], (unsigned char)rot);
				w[14] = bit_rotate_right(w[14], (unsigned char)rot);
				w[15] = bit_rotate_right(w[15], (unsigned char)rot);
			}
		}

		for (size_t i = 0; i < base_t::digest_digits; i++)
			base_t::m_result[i] ^= w[15 - i];
	}
	...
	void terminate()
	{
		...
		// Pad whole block if partially filled
		if (base_t::m_digitProgress != 0)
			process_digit();

		while (base_t::m_blockProgress != 0)
			process_digit();

		for (size_t i = 0; i < base_t::stride_digits - 2; i++)
			m_state[i] = 0;
		m_state[base_t::stride_digits - 2] = (uint32_t)(bitCount >> 32);
		m_state[base_t::stride_digits - 1] = (uint32_t)bitCount;
		process_block();
	}
...
template <>
class snefru<128> : public snefru_base<128, 384>
{
private:
	typedef snefru_base<128, 384> base_t;
...
template <>
class snefru<256> : public snefru_base<256, 256>
{
private:
	typedef snefru_base<256, 256> base_t;
