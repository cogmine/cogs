//
//  Copyright (C) 2000-2020 - Colen M. Garoutte-Carson <colen at cogmine.com>, Cog Mine LLC
//


// Status: Good

#ifndef COGS_HEADER_CRYPTO_MD2
#define COGS_HEADER_CRYPTO_MD2


#include "cogs/crypto/serial_hash.hpp"
#include "cogs/math/bytes_to_int.hpp"
#include "cogs/mem/endian.hpp"


namespace cogs {
namespace crypto {

#ifdef DOXYGEN

/// @ingroup Crypto
/// @brief MD2 Message-Digest Algorithm
class md2 : public hash
{
};

#else

class md2 : public serial_hash<384, 128, 8, endian_t::little, 128>
{
private:
	typedef serial_hash<384, 128, 8, endian_t::little, 128> base_t;

	uint8_t m_checksum[16];

	void process_digit()
	{
		m_result[base_t::m_blockProgress + 16] = base_t::m_curDigit;
		m_result[base_t::m_blockProgress + 32] = m_result[base_t::m_blockProgress] ^ base_t::m_curDigit;
		base_t::advance_digit();
	}

	void process_block()
	{
		static constexpr uint8_t S[256] = {
			 };

		// Update checksum.
		uint8_t t = m_checksum[15];
		for (uint8_t i = 0; i < 16; i++)
			t = m_checksum[i] ^= S[m_result[i + 16] ^ t];

		// Encrypt block (18 rounds).
		t = 0;
		for (uint8_t i = 0; i < 18; i++)
		{
			for (uint8_t j = 0; j < 48; j++)
				t = (m_result[j] ^= S[t]);
			t = t + i;
		}
	}

	void terminate()
	{
		uint8_t x = (uint8_t)16 - (uint8_t)m_blockProgress;
		for (uint8_t i = (uint8_t)m_blockProgress; i < 16; i++)
		{
			m_result[i + 16] = x;
			m_result[i + 32] = m_result[i] ^ x;
		}
		process_block();

		for (uint8_t i = 0; i < 16; i++)
		{
			x = m_checksum[i];
			m_result[i + 16] = x;
			m_result[i + 32] = m_result[i] ^ x;
		}
		process_block();
	}

public:
	md2()
		: base_t([&]() { process_digit(); }, [&]() { process_block(); }, [&]() { terminate(); })
	{
		for (size_t i = 0; i < 16; i++)
		{
			m_checksum[i] = 0;
			m_result[i] = 0;
		}
	}

	md2(const md2& src)
		: base_t(src, [&]() { process_digit(); }, [&]() { process_block(); }, [&]() { terminate(); })
	{
		size_t i = 0;
		for (; i < m_blockProgress; i++)
		{
			m_result[i] = src.m_result[i];
			m_result[i + 16] = src.m_result[i + 16];
			m_result[i + 32] = src.m_result[i + 32];
			m_checksum[i] = src.m_checksum[i];
		}
		for (; i < 16; i++)
		{
			m_checksum[i] = src.m_checksum[i];
			m_result[i] = src.m_result[i];
		}
	}

	md2& operator=(const md2& src)
	{
		base_t::operator=(src);
		size_t i = 0;
		for (; i < m_blockProgress; i++)
		{
			m_result[i] = src.m_result[i];
			m_result[i + 16] = src.m_result[i + 16];
			m_result[i + 32] = src.m_result[i + 32];
			m_checksum[i] = src.m_checksum[i];
		}
		for (; i < 16; i++)
		{
			m_checksum[i] = src.m_checksum[i];
			m_result[i] = src.m_result[i];
		}

		return *this;
	}
};

#endif

}
}


#endif
